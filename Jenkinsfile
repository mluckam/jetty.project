      ...
      parallel {
        ...
        stage("Build / Test - JDK17") {
          ...
          steps {
            timeout( time: 180, unit: 'MINUTES' ) {
              checkout scm
              mavenBuild( "jdk17", "clean install -Perrorprone", "maven-3.9.x") // javadoc:javadoc
    ...
    try {
      withEnv(["JAVA_HOME=${ tool "$jdk" }",
        ...
        configFileProvider(
          ...
          sh "mvn -Dmaven.repo.uri=http://nexus-service.nexus.svc.cluster.local:8081/repository/maven-public/ -Dmaven.test.failure.ignore=true -ntp -s $GLOBAL_MVN_SETTINGS -Dmaven.repo.local=.repository -Pci -DexcludedGroups=\"external, large-disk-resource, stress, slow, not-on-ci, flaky\" -V -B -e -Djetty.testtracker.log=true -Dsurefire.rerunFailingTestsCount=1 $cmdline"
        }
      }
    }
    finally
